...
class SFact:
    ...
    def __neg__(self):
        return LiftF(
            "neg", lambda x: -x, [self], types=[self.outType], outType=self.outType
        )
...
class LiftF(SFact):
    ...
    def __str__(self):
        return "{0} - args: {1} - types: {2} - outType: {3}".format(
            str(self.name), map(str, self.args), map(str, self.types), str(self.outType)
        )
...
def eventObserver(eName, eVal=None):
    def getEvent(ename):
        ...
        if ename in Globals.events:
            ...
            return (
                EventValue(Globals.events[ename]) if eVal is None else EventValue(eVal)
            )
