...
class Proxy:
    ...
    def _get(self, name):
        ...
        print(
            str(name)
            + " does not exist or has not been started in this Proxy: "
            + repr(self)
        )
    ...
    def _initialize(self):
        for k, v in self._updateSignals.items():
            ...
            v = maybeLift(v)
            Globals.error = (
                "On Line 51 of Proxy, In object " + k + ", attribute " + str(v)
            )
    ...
    def _react1(self, when, what):
        if self._alive:
            ...
            Globals.error = (
                "On Line 59 of Proxy, In object "
                + self._name
                + ", initializing one time reaction "
                + when.name
            )
    ...
    def _update(self):
        if self._alive:
            ...
            if len(thunks) >= 2:
                print(
                    "Multiple one time reactions in a heartbeat in object " + self._name
                )
