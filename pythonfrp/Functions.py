...
def integralInternal(
    x,
):  # This reurns the special value Zero() at the initial sampling.
...
def tagMap(fn, evt):
    return lift("tag", lambda e: EventValue(fn(e.value)) if e.occurs() else noEvent)(
        evt
    )
...
def changes(e):
    def initChanges(sm):
        ...
        sm.last = (
            EventValue()
        )  # This is any value tha is guaranteed not to be in the input
